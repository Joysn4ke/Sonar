...
void SERVO_rotation(void) {
    ...
    position = (rotation) ? (position + 10) : (position - 10);      // Incr�menter ou d�cr�menter position en fonction de rotation

    position = (position < 0) ? (0) : (position);                   // Reset position pour éviter overflow
