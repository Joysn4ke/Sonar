...
/**
 * @brief Fonction permettant de récupérer une chaîne de caractère contenant l'identifiant unique d'une carte.
 * @retval cardUID : identifiant de la carte sous forme d'une chaîne de caractères
 */
char* getUID_String(void){
	int i = 0;
	char uid[] = "UID (dec) : "; //"UID (hex) : ";
	char dataOut[128];
	char *cardUID = NULL;

	for (i = 0; i < ISO14443A_Card.UIDsize; i++){
		/*
		 * Pour un ID en hexadecimal, remplacer le %u par %x
		 */
		sprintf(dataOut, "%s%u ", uid, ISO14443A_Card.UID[i]);
		sprintf(uid, "%s", dataOut);
	}
	sprintf(dataOut, "%s%s ", uid, "- END UID");

	cardUID = malloc((strlen(dataOut)+1)*sizeof(char));
	strcpy(cardUID, dataOut);
...
/**
 * @brief Fonction permettant de récupérer un pointeur sur tableau contenant l'identifiant unique d'une carte.
 * @retval ptr : pointeur sur tableau contenant l'ID de la carte, la première valeur correspond à la taille de l'ID.
 */
uint8_t * getUID(void) {
	int i = 1;
	uint8_t *ptr;
	uint8_t tab[ISO14443A_Card.UIDsize+1];

	tab[0] = ISO14443A_Card.UIDsize;

	for (i = 1; i < ISO14443A_Card.UIDsize+1; i++)
		tab[i] = ISO14443A_Card.UID[i-1];
